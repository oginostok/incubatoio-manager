...
import { ProductionFarmsAPI, type ProductionFarm } from "@/lib/api";
...
export default function ProductionFarmsSettings({ onChanged }: Props) {
    const [farms, setFarms] = useState<ProductionFarm[]>([]);
    ...
    const load = async () => {
        setLoading(true);
        try {
            const data = await ProductionFarmsAPI.getAll();
            setFarms(data.farms);
        } finally {
            setLoading(false);
        }
    ...
    const handleRename = async (farm: ProductionFarm, nome: string) => {
        const trimmed = nome.trim();
        if (!trimmed || trimmed === farm.nome) return;
        try {
            await ProductionFarmsAPI.update(farm.id, { nome: trimmed });
            await refresh();
        } catch (e: any) {
    ...
    const handleCapannoni = async (farm: ProductionFarm, n: number) => {
        const val = Math.max(1, Math.round(n || 1));
        if (val === farm.n_capannoni) return;
        await ProductionFarmsAPI.update(farm.id, { n_capannoni: val });
        await refresh();
    };

    const handleDelete = async (farm: ProductionFarm) => {
        if (!confirm(`Eliminare l'allevamento "${farm.nome}"?`)) return;
        try {
            await ProductionFarmsAPI.remove(farm.id);
            await refresh();
        } catch (e: any) {
            setError(e?.response?.data?.detail || "Errore nell'eliminazione dell'allevamento");
        }
    };
    ...
    const handleAdd = async () => {
        ...
        try {
            await ProductionFarmsAPI.add(nome, Math.max(1, Math.round(newCap || 1)));
    ...
    return (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 mb-6">
            ...
            <p className="text-sm text-gray-500 mb-4">
                Aggiungi o togli allevamenti, modifica il nome o il numero di capannoni.
            </p>
                    ...
                    <table className="w-full text-sm" style={{ maxWidth: 640 }}>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td className="px-3 py-2 text-center">
                                    <Button type="button" size="sm" onClick={handleAdd} className="bg-red-600 hover:bg-red-700">
