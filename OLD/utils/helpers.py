...
from database import init_db, get_lotti, add_lotto, engine
import sqlalchemy

@st.cache_data
def carica_dati_v20():
    """Carica dati da DB (standard_curves) o da CSV se tabella manca (seeding)."""
    try:
        # 1. Tenta lettura da DB
        try:
            df = pd.read_sql("standard_curves", engine)
            if not df.empty:
                # FIX: Normalizza spazi (nel caso nel DB ci siano doppi spazi vecchi)
                df.columns = df.columns.str.replace(r'\s+', ' ', regex=True).str.strip()
                return df
        except:
            pass # Tabella non esiste o errore, fallback su CSV
        ...
        # Rimpiazza newline e spazi multipli con uno spazio singolo
        df.columns = df.columns.str.replace(r'\s+', ' ', regex=True).str.strip()
        
        # 3. Salva nel DB per il futuro
        try:
            df.to_sql("standard_curves", engine, if_exists='replace', index=False)
        except Exception as e:
            print(f"Errore salvataggio DB standard_curves: {e}")
